...
docker(docker), useThreads(useThreads), maxWorkersPerBatch(maxWorkersPerBatch), maxMemoryPerWorker(maxMemoryPerWorker), maxCpuTimePerWorker(maxCpuTimePerWorker), workerTimeout(workerTimeout), batchTimeout(batchTimeout), maxWorkers(maxWorkers){
}
