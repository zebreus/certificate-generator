...
#include "Server.hpp"
  ...
  void CertificateGeneratorHandler::addResourceFile(const File& resourceFile) {
	//Save file to disk
    ...
    if(!resourceFileStream){
		stringstream message;
		message << "Failed to write resourceFile.";
  ...
  void CertificateGeneratorHandler::addTemplateFile(const File& templateFile) {
	//Save file to disk
  ...
  void CertificateGeneratorHandler::generateCertificates(std::vector<File> & _return) {
		//TODO new output directory for every generateCertificates from the same connection
